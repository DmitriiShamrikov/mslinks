...
public class Registry {
	...
	public static void registerClsid(GUID clsid, String name, Class<?>... allowedItemIdTypes) {
		registerClsid(clsid, name, false, allowedItemIdTypes);
	}

	public static void registerClsid(GUID clsid, String name, boolean isControlClsid, Class<?>... allowedItemIdTypes) {
		...
		if (entry != null) {
			...
			entry.isControl = isControlClsid;
			return;
		}

		registerClsidInternal(clsid, name, isControlClsid, allowedItemIdTypes);
	}

	private static GUID registerClsid(String clsid, String name, Class<?>... allowedItemIdTypes) {
		return registerClsid(clsid, name, false, allowedItemIdTypes);
	}

	private static GUID registerClsid(String clsid, String name, boolean isControlClsid, Class<?>... allowedItemIdTypes) {
		var guid = new GUID(clsid);
		registerClsidInternal(guid, name, isControlClsid, allowedItemIdTypes);
		return guid;
	}

	private static void registerClsidInternal(GUID clsid, String name, boolean isControlClsid, Class<?>... allowedItemIdTypes) {
		var entry = new Entry();
		entry.clsid = clsid;
		entry.name = name;
		entry.isControl = isControlClsid;
	...
	static {
		...
		CLSID_USERFOLDER =
		registerClsid("{59031A47-3F72-44A7-89C5-5595FE6B30EE}", "UserFolder", true);
		registerClsid("{9C73F5E5-7AE7-4E32-A8E8-8D23B85255BF}", "UserFolder", true);
